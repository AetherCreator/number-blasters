// WordQuest.jsx — falling-word arcade game (clue-2 of word-quest hunt)
// Forked from NumberBlasters.jsx — same arcade rhythm, reads from word-pack-v1.json.
// Mechanic: prompt at top ("means: <definition>"), 4 word-asteroids fall, tap correct.
// MVP: random word selection. Clue-3 layers adaptive difficulty + audio polish.

import { useState, useEffect, useCallback, useRef, useMemo } from "react";
import wordPack from "./data/word-pack-v1.json";

const STAR_COUNT = 60;
const GAME_DURATION = 60;
const MAX_LIVES = 3;

// Word Quest — single difficulty for MVP, adaptive layer added in Clue 3
const DIFFICULTIES = {
  cadet: { label: "Cadet", speed: 9000, emoji: "📖" },
};
// Pull all words once; clue-3 will filter by adaptive currentDifficulty
...
export default function WordQuest({ onExit }) {
  ...
  const handleSelect = (choice) => {
    ...
    if (problem.choices.indexOf(choice) === problem.correctIndex) {
      ...
      setCombo(1);
      setShake(true);
      setFeedback({ text: `= ${problem.choices[problem.correctIndex]}`, color: "#FF6B6B" });
      setTimeout(() => setShake(false), 400);
